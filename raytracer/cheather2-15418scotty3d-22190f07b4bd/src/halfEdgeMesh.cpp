...
float Vertex::laplacian() const
{
  ...
  HalfedgeIter first = _halfedge;
  HalfedgeIter h = first;
  do {
    float cotanWeight = 0.0;
    VertexIter u = h->twin()->vertex();
    Vector3D U = u->position;
    Vector3D A = h->next()->next()->vertex()->position;
    Vector3D B = h->twin()->next()->next()->vertex()->position;
    Vector3D e1 = U - A;
    Vector3D e2 = position - A;
    if (!h->isBoundary()) cotanWeight += dot(e1, e2) / cross(e1, e2).norm();
    e1 = U - B;
    e2 = position - B;
    if (!h->twin()->isBoundary()) cotanWeight += dot(e1, e2) / cross(e1, e2).norm();
    sum += 0.5 * cotanWeight * (u->offset - offset);
    h = h->twin()->next();
  } while (h != first);
