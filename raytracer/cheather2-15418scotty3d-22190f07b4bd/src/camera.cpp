...
Ray Camera::generate_ray(double x, double y) const {
  ...
  double cam_x = (2*x - 1) * tan(radians(hFov)/2);
  double cam_y = (2*y - 1) * tan(radians(vFov)/2);
  Vector3D cam_sample_dir = Vector3D(cam_x, cam_y, -1.0);
  Vector3D world_sample_dir = c2w * cam_sample_dir;
  

  world_sample_dir.normalize();
  return Ray(position(), world_sample_dir);
}
