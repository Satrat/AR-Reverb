  ...
  void Skeleton::unkeyframe(double t) {
   ...
   void Skeleton :: reachForTarget(map<Joint*, Vector3D> targets, double time )
   {
      ...
      for( int iter = 0; iter < nIterations; iter++ )
      {
         ...
         for( vector<Joint*>::iterator j = joints.begin(); j != joints.end(); j++ )
         {
            (*j)->ikAngleGradient = Vector3D(0, 0, 0);
         }

         for (auto x : targets) {
           auto goalJoint = x.first;
           auto targetPoint = x.second;
           goalJoint->calculateAngleGradient(goalJoint, targetPoint);
           root->ikAngleGradient = Vector3D(0, 0, 0); // Dont' want to rotate the root.
         }

         // For each joint, take a small step in the direction of the energy gradient.
         double tau = 1.0; // initial timestep

         float cost = 0.0f;
         for (auto x : targets) {
           auto goalJoint = x.first;
           auto targetPoint = x.second;
           cost += (goalJoint->getEndPosInWorld() - targetPoint).norm2();
         }

         while (1) {
           for( vector<Joint*>::iterator j = joints.begin(); j != joints.end(); j++ )
           {
              Vector3D theta = (*j)->getAngle( time );
              (*j)->setAngle( time, theta - tau * (*j)->ikAngleGradient );
              (*j)->rotation = (*j)->getAngle(time);
           }
           float sum = 0.0f;
           for (auto x : targets) {
             auto goalJoint = x.first;
             auto targetPoint = x.second;
             sum += (goalJoint->getEndPosInWorld() - targetPoint).norm2();
           }
           if (sum > cost) {
             for( vector<Joint*>::iterator j = joints.begin(); j != joints.end(); j++ )
             {
