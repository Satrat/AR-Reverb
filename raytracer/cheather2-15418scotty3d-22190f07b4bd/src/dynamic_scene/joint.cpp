...
namespace CMU462 { namespace DynamicScene {
   ...
   {
     Vector3D obj = goalJoint->getEndPosInWorld() - q;
     Vector3D v = goalJoint->getEndPosInWorld() - getBasePosInWorld();
     Matrix4x4 trans = getTransformation();

     Vector3D axisX = (trans * Vector4D(1,0,0,0)).to3D();
     Vector3D gradientX = cross(axisX, v);
     ikAngleGradient.x += dot(gradientX, obj);

     Vector3D axisY = (trans * Vector4D(0,1,0,0)).to3D();
     Vector3D gradientY = cross(axisY, v);
     ikAngleGradient.y += dot(gradientY, obj);
 
     Vector3D axisZ = (trans * Vector4D(0,0,1,0)).to3D();
     Vector3D gradientZ = cross(axisZ, v);
     ikAngleGradient.z += dot(gradientZ, obj);

     if (parent != nullptr) parent->calculateAngleGradient(goalJoint, q);
  }
   ...
   Vector3D Joint::getBasePosInWorld()
   {
     ...
     */
     Vector4D basePos4d(0., 0., 0., 1.);
     Matrix4x4 T = getTransformation();

     return (T * basePos4d).projectTo3D();
   }
   ...
   Vector3D Joint::getEndPosInWorld()
   {
     ...
     coordinate frame.
     */
     Vector4D endPos4d(0,0,0,1);
     Matrix4x4 T = getTransformation() * this->SceneObject::getTransformation() * Matrix4x4::translation(axis);

     return (T * endPos4d).projectTo3D();
   }
