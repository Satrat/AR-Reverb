...
/* TODO Not given out to students */
Vector3D closestPointOnLineSegment(Vector3D p1, Vector3D p2, Vector3D p)
{
  Vector3D m = p2 - p1;
  if (m.norm() < 1E-6) return p1;

  double t0 = dot(m, (p - p1)) / dot(m, m);
...
{
  /* TODO */
  map<VertexIter, vector<LBSInfo>> lbsInfo;

  for (VertexIter v = mesh.verticesBegin(); v != mesh.verticesEnd(); v++)
  {
    for (Joint* joint : skeleton->joints)
    {
      // Check if projection of vertex position falls on the line segment 
      // defined by b + t0 * m where t0 is restricted to [0, 1].
      Vector3D p = v->bindPosition;
      Matrix4x4 bTransform = joint->getBindTransformation();
      Vector3D p1 = bTransform * Vector3D(0, 0, 0);
      Vector3D p2 = p1 + joint->axis;
      Vector3D closestPoint = closestPointOnLineSegment(p1, p2, p);
      double r = joint->capsuleRadius;
      double d = (closestPoint - p).norm();

      if (d < r || !useCapsuleRadius)
      {
        // Store result
        LBSInfo info;
        info.distance = d;
        info.blendPos = getTransformation().inv() * joint->getTransformation() * joint->SceneObject::getTransformation() * (p - p1);
        lbsInfo[v].push_back(info);
      }
    }
  }
  ...
  for (VertexIter v = mesh.verticesBegin(); v != mesh.verticesEnd(); v++)
  {
    vector<LBSInfo> &vertexLBSInfo = lbsInfo[v];
    if (vertexLBSInfo.size() == 0)
    {
      v->position = v->bindPosition;
      continue;
    }

    // Compute the actual weight which should be inversely proportional to the distance
    double totalInvWeight = 0;
    for (LBSInfo info : vertexLBSInfo)
      totalInvWeight += 1 / info.distance;

    double n = 1 / totalInvWeight;

    // Compute the weighted average of the vertex position
    Vector3D pos(0, 0, 0);
    double check = 0;
    for (LBSInfo info : vertexLBSInfo)
    {
      pos += info.blendPos * n / info.distance;
      check += n / info.distance;
    }
    assert(abs(check - 1.0) < 0.01);
...
void Mesh::forward_euler(float timestep, float damping_factor) {
  ...
  vector<float> offs;
  for (VertexIter v = mesh.verticesBegin(); v != mesh.verticesEnd(); v++) {
    float vel = (v->velocity * (1.0 - timestep*damping_factor)) + (timestep * v->laplacian());
    vels.push_back(vel);
    offs.push_back(v->offset + timestep * v->velocity);
  }
...
void Mesh::symplectic_euler(float timestep, float damping_factor) {
  ...
  for (VertexIter v = mesh.verticesBegin(); v != mesh.verticesEnd(); v++) {
    float vel = (v->velocity * (1.0 - timestep*damping_factor)) + (timestep * v->laplacian());
    vels.push_back(vel);
    offs.push_back(v->offset + timestep * vel);
  }
