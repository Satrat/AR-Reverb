...
Spectrum PathTracer::trace_ray(const Ray &r) {
  ...
  Intersection isect;
  Spectrum L_out = Spectrum(0.f, 0.f, 0.f);
  ...
  if (bvh->intersect(r, &isect)) 
  {
       ...
       for(SceneLight* light : scene->lights)
       {
          ...
          for(int i=0; i<num_light_samples; i++)
          {
             ...
             Ray shadowRay = Ray(o, dir_to_light, dist_to_light);
             Intersection i_shadow;
             if(!bvh->intersect(shadowRay, &i_shadow))
             {
                
               
                
                L_direct += light_L * f * cos_theta * (1 / pdf) * scale;
             }
             else
             {
                // log ray hit
                #ifdef ENABLE_RAY_LOGGING
                log_ray_hit(shadowRay, i_shadow.t);
                #endif
             }
          }
       }
       ...
       //Spectrum f_test = isect.bsdf->f(w_out, w_in);
       //potentially kill path (using Russian roulette)
     
       float terminateProbability = 1.0 - 3.0*f.illum();
       if(terminateProbability < 0.0)
       {
          terminateProbability = 0.0;
       }
       //Generate a random number between 0 and 1
       Vector2D random2D = gridSampler->get_sample();
       
       if((random2D.x < terminateProbability || r.depth <= 0))
       {
         return L_out;
       }
       ...
       Vector3D direction_to_next_surface = o2w * w_in;
       
       //Needs to be absolute value, incase of refraction
       double cos_theta = abs(w_in[2]);
      
       Vector3D o_bounce = hit_p + EPS_D * direction_to_next_surface;
       Ray bounceRay = Ray(o_bounce, direction_to_next_surface);
       
       bounceRay.depth = r.depth - 1;
          
       Spectrum additional_light =
            trace_ray(bounceRay) * cos_theta * f * (1 / (pdf * (1-terminateProbability)));
        
        
       return L_out + additional_light;
  }
...
Spectrum PathTracer::raytrace_pixel(size_t x, size_t y) {
  ...
  float w = (double) sampleBuffer.w;
  float h = (double) sampleBuffer.h;
  
  
  
  float x_f = (((double) x) + 0.5) / frameBuffer.w;
  
  float y_f = (((double) y) + 0.5) / frameBuffer.h;
  Vector2D p = Vector2D(x_f, y_f);
  
  Spectrum L = trace_ray(camera->generate_ray(p.x, p.y));
  for(int i = 1; i < num_samples; i++)
  {
    Vector2D d = gridSampler->get_sample();
    x_f = (((double) x) + d.x) / frameBuffer.w;
  
    y_f = (((double) y) + d.y) / frameBuffer.h;
    p = Vector2D(x_f, y_f);
    Ray r = camera->generate_ray(p.x, p.y);
    r.depth = max_ray_depth;
    //L += trace_ray(camera->generate_ray(p.x, p.y));
    L += trace_ray(r);
  }
  L = L * (1.0 / num_samples);
  return L;
